...
Full reference of Desk Support Monkey API endpoints. Source: `https://staging.desksupportmonkey.com/openapi.json`.

> **Note:** API responses are untyped in the OpenAPI spec. Response shapes must be inferred from actual API calls and typed with Zod schemas in the frontend.
...
### Pagination
All list endpoints accept `page` (1-based, default 1) and `page_size` (default varies, typically 20-50).

### Error responses
- `422` — Validation error: `{ detail: [{ loc: [], msg: string, type: string }] }`
...
### Status transitions
Several entities use state machines (requests, purchase orders, shipments, maintenance, appointments). Status changes are done via dedicated action endpoints (e.g., `/submit`, `/approve`, `/cancel`), not by directly setting the status field.
